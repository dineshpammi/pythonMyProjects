...
3.ACCEPT FROM THE USER: DAYS IN FUTURE
4.DISPLAY: FUTURE DAY+SUPERSCRIPT(st,nd,rd,th), MONTHNAME, YEAR
'''
...
y=int(y)
while 1:
    m=input('Month?') or '5'        #default to 5                      
    m=int(m)
    if (m >= 1 and m <= 12):
        if y%4 and m==2:
            md[m]-=1
        d=input('day ?') or '29'    #default to 29            
        d=int(d)
        if (d > 0 and d <= md[m]):
            break
...
print(daysAfter, 'days later')
print(fd,' / ',fm,' / ',fy)
print(fd,ss[fd],mn[fm],fy)
